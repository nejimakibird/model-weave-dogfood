...
| 参照元ファイルパス | sourceFilePath | string | | | Y | N | N | | 参照が記述されたファイル |
| 参照元セクション | sourceSection | string | | | N | N | N | | Fields / Mappings / Notes 等 |
| 参照元項目 | sourceField | string | | | N | N | N | | ref / rule / source_ref 等 |
| 参照文字列 | referenceText | string | | | Y | N | N | | 記述された参照値 |
| 参照先アセットID | targetAssetId | string | | | N | N | N | | 解決できた場合 |
| 参照先ファイルパス | targetFilePath | string | | | N | N | N | | 解決できた場合 |
| 参照種別 | referenceKind | string | | | Y | N | N | | wikilink / id / source_link 等 |
...
|---|---|---|---|
| source/model-weave-repo/src/models/core/reference-resolver.ts | Reference | type | 参照情報の内部表現 (未確認) |