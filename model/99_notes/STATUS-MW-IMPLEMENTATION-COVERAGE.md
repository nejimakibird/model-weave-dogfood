...
この版では Model Weave 0.1.20 の source / tests / docs を基準に、app_process Business Flow、Flow Diagram Detail / Screen view、Mapping target-first canonical header、diagnostics guidance、Color Scheme、Domain Sources、Source Links、Impact / Relationship View、Weave Map、Settings、Render Mode の扱いを再整理する。dogfoodがどの本体リリースまで反映済みかは [[STATUS-MW-DOGFOOD-BASELINE]] で管理する。
...
| implemented | Flow Diagram MVP | `type: flow_diagram`、`kind: screen_communication`、Objects / Flows解析、Detail view描画、node / edge hover、`Flows.data` Wikilink診断 |
| implemented | Flow Diagram Domain Sources / local Domains | `## Domain Sources` とlocal `## Domains` を解析し、Domain name group label、parent nested group、kind color keyへ反映する |
| implemented | Flow Diagram Detail / Screen view | Viewer toolbarのFlow view selector、frontmatter `flow_view` 初期mode、Settings `defaultFlowDiagramViewMode`、Screen projection、file scoped state |
| implemented | Flow Diagram Screen projection | screen / external / actor / user / context / message をvisible nodeとして残し、内部node経由の到達関係をprojected edgeにする |
| implemented | Mapping target-first canonical format | `target_ref / source_ref / transform / rule / required / notes` を現行canonical headerとして扱う |
| implemented | Mapping legacy source-first compatibility | legacy `source_ref / target_ref / transform / rule / required / notes` は警告なしで受理されるが、dogfood実テーブルはtarget-firstへ統一する |
| implemented | Flow Diagram dogfood representative sample | [[FLOW-MW-VIEWER-DIAGNOSTIC-QUICK-FIX]] を追加済み。Quick Fix MVPの画面、UI action、frontmatter patch、保存、Preview再評価のcommunication flowを示す |
...
| implemented | stable lower panel tabs | Review情報をDetails / Relationships / Diagnostics / Source Links / Mermaidの安定tabへまとめる。graph / non-graph review view、renderer switching、Business Flow direction変更後の表示維持を含む。[[DATA-MW-LOWER-PANEL-TAB-STATE]] に反映済み |
...
| implemented | v0.1.18 dogfood catch-up | Lower panel tabs、diagnostics bulk copy、manual guidance、Quick Fix MVP、empty table row handlingはCOV-014A〜COV-014Eで反映済み |
...
| future | Flow Diagram arbitrary projections / Communication View | v0.1.20のScreen projectionとは別。任意projection定義、generic folding、transition coverage生成、state machine生成はfuture |
...
| 0.1.19 | Flow Diagram MVP, diagnostics guidance, Color Scheme preview editing, Applied Color Scheme compact display | implemented / future | Flow Diagram MVPは [[DATA-MW-FLOW-DIAGRAM-PARSED-DIAGRAM]]、[[PROC-MW-FLOW-DIAGRAM-PARSE]]、[[PROC-MW-FLOW-DIAGRAM-RENDER]]、[[FLOW-MW-VIEWER-DIAGNOSTIC-QUICK-FIX]] に反映済み。当時MVP外だったScreen projectionはv0.1.20でimplementedへ移動 |
| 0.1.20 | Mapping target-first, Flow Diagram Domains, Detail / Screen view, Flow view state | implemented / future | target-first Mapping、Flow Diagram Domain Sources、Screen projection、Flow view selector、flow_view initial mode、defaultFlowDiagramViewMode、initializationKey stateを反映済み。任意projectionやgeneric foldingはfuture |
...
- Flow Diagramは実装済みとして扱う。ただし `dfd_diagram` とは別形式であり、`type: flow_diagram` のDetail / Screen viewをimplementedとする。任意projection定義、Communication View、generic folding、transition coverage生成はfutureである。
- `flow_view` は初期表示入力であり、toolbar切替後のViewer stateやMarkdown本文を固定しない。
- Mappingの現行canonical headerはtarget-firstである。legacy source-firstは互換として受理されるが、新規dogfood実テーブルでは使わない。
- Flow Diagramの代表サンプルとして [[FLOW-MW-VIEWER-DIAGNOSTIC-QUICK-FIX]] と [[FLOW-MW-FLOW-DIAGRAM-VIEW-MODE]] を追加済みである。これらはscreen communicationとview modeの流れを示すものであり、DFD、Explorer、任意projection定義、自動生成機能ではない。
...
- v0.1.18 stable lower panel tabsは実装済みとして扱うが、Viewer内Review UIであり、Source Links Explorer / Reference Explorer / Model Index Viewではない。Markdown本文を変更せず、PNG export対象にも含めない。
