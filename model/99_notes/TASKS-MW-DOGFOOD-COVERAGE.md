...
| COV-013 | v0.1.19 release alignment | 反映済み | high | v0.1.19のFlow Diagram MVP、diagnostics guidance、Color Scheme編集をdogfoodへ反映できているか | v0.1.19 release notes, FORMAT-flow_diagram, dfd-diagram-parser.ts, relation-resolver.ts, dfd-mermaid.ts, diagnostic-section-guidance.ts, color-scheme-table-editor.ts, applied-color-scheme-renderer.ts | 03_data, 04_process, 05_rule, 06_mapping, 09_screen, 99_notes | Flow Diagram MVPはtype: flow_diagram、kind: screen_communication、Objects / Flows表、ローカルendpoint、Flows.data Wikilink診断、Detail viewとして実装済み。Color Scheme preview editingとApplied Color Scheme compact tableも実装済み | Flow Diagram DATA / parse / render processを追加し、既存Parser / Resolver / Renderer / Diagnostics / Viewerへ最小反映 | COV-013Aで代表Flow Diagramサンプルやdogfood実例が必要か判断する | 当時futureだったScreen projectionはCOV-015でimplementedへ移動 |
| COV-013A | Flow Diagram sample and model coverage | 反映済み | medium | dogfood内にFlow Diagramの代表モデルやサンプルを追加する必要があるか | FORMAT-flow_diagram, dfd-diagram-parser.ts, modeling-preview-view.ts | 10_flow_diagram | Quick Fix MVPの画面、UI action、frontmatter patch、保存、Preview再評価を代表Flow Diagramとして追加 | [[FLOW-MW-VIEWER-DIAGNOSTIC-QUICK-FIX]] を追加し、DFDやExplorerではないscreen communication例として扱う | 今後Flow Diagram warningが出た場合だけ個別に補正する | v0.1.20のview mode代表例はCOV-015で追加 |
...
| COV-014 | dogfood baseline and v0.1.18 catch-up | 反映済み | high | dogfoodがどの本体releaseまで反映済みか明示し、v0.1.18要素の取りこぼしを追跡できているか | v0.1.18 release notes, modeling-preview-view.ts, markdown-table.ts | STATUS-MW-DOGFOOD-BASELINE, STATUS, TASKS | v0.1.18のlower panel tabs、diagnostics bulk copy、manual guidance、Quick Fix MVP、empty table row handlingをCOV-014A〜Eで確認 | baseline noteと各dogfood modelへ反映済み | 次はCOV-013Aまたは未反映の低優先taskへ進む | implementedはsrc確認済みの範囲だけ。docsやrelease notesだけではimplemented化しない |
| COV-014A | v0.1.18 lower panel tabs | 反映済み | medium | lower panel tabsのstable behaviorをscreen / data / mappingへどこまで反映するか | modeling-preview-view.ts | DATA-MW-LOWER-PANEL-TAB-STATE, SCR-MW-VIEWER-MAIN-FRAME, DATA-MW-VIEWER-STATE, MAP-MW-VIEWER-STATE-TO-MAIN-FRAME-UI | `.model-weave-lower-tabs`、activeLowerPanelTabId、graph / non-graph review view、renderer switching後の維持はsource-backedに確認できる | lower panel tab stateをDATA化し、Viewer Main Frame / mapping / diagnostics位置づけへ反映 | COV-014 catch-upを完了扱いにする | Explorer系future機能ではなくViewer内Review UIとして扱う |
| COV-014B | v0.1.18 diagnostics copy actions | 反映済み | medium | all diagnostics / errors / warnings / notesのbulk Markdown copyをDiagnostics screen / mappingへ反映するか | modeling-preview-view.ts, i18n messages | DATA-MW-DIAGNOSTIC-COPY-ACTION, SCR-MW-VIEWER-DIAGNOSTICS-PANEL, MAP-MW-RENDERER-DIAGNOSTIC-TO-UI | `diagnostics.copyAll`、`copyErrors`、`copyWarnings`、`copyNotes` はsource-backedに確認できる | bulk Markdown copyをDATA / SCREEN / PROCESS / MAPPINGへ反映 | COV-014Cへ進む | AI/chatレビュー向けのコピー支援。Quick FixやMarkdown本文修正ではない |
| COV-014C | v0.1.18 manual-edit guidance | 反映済み | medium | diagnostic detailsのmanual repair guidanceをRULE / SCREENへどう整理するか | modeling-preview-view.ts, diagnostic-section-guidance.ts, i18n messages | DATA-MW-DIAGNOSTIC-MANUAL-GUIDANCE, RULE-MW-DIAGNOSTIC-GUIDANCE-DERIVATION, SCR-MW-VIEWER-DIAGNOSTICS-PANEL | reference、expected header、frontmatter example、what happened / cause / manual fixはsource-backed | 手動修復guidanceをDATA / RULE / SCREEN / PROCESS / MAPPINGへ反映 | COV-014Eへ進む | Quick FixやMarkdown本文自動修正ではない |
...
| COV-014E | v0.1.18 Markdown table rows | 反映済み | medium | fully empty data row ignoreとpartially filled row validationをDATA / RULE / parser notesへ反映するか | markdown-table.ts, FORMAT-common-sections.md | DATA-MW-CORE-MARKDOWN-TABLE, RULE-MW-PARSER-SECTION-STRUCTURE, PROC-MW-PARSER-PARSE-VAULT-FILE | `isEmptyMarkdownTableDataRow` により全セル空のdata rowだけ無視する | fully empty row除外とpartial row診断維持をDATA / RULE / PROCESSへ反映 | COV-014Aへ戻る | 部分入力行はrequired-field diagnostics対象のまま |
| COV-015 | v0.1.20 Mapping / Flow Diagram alignment | 反映済み | high | v0.1.20のMapping target-firstとFlow Diagram Detail / Screen viewをdogfoodへ反映できているか | v0.1.20 release notes, mapping-parser.ts, flow-diagram-view-mode.ts, dfd-diagram-parser.ts, dfd-mermaid.ts, modeling-preview-view.ts, flow-diagram tests | 03_data, 04_process, 05_rule, 06_mapping, 09_screen, 10_flow_diagram, 99_notes | Mapping canonical headerはtarget-first。Flow DiagramはDomain Sources、local Domains、flow_view、defaultFlowDiagramViewMode、file scoped view state、Screen projectionを実装済み | 実Mapping tablesをtarget-firstへ移行し、Flow Diagram DATA / RULE / PROCESS / SCREEN / representative sampleへ反映 | COV-015Aで必要に応じて実機PreviewでDetail / Screen表示差分を確認する | arbitrary projection、generic folding、Communication view、transition coverage generationはfuture維持 |
| COV-015A | Flow Diagram view mode実機確認 | 要追加調査 | low | representative Flow DiagramでDetail / Screen view、Domain group、Color Scheme表示が期待どおりか | Model Weave preview, FLOW-MW-FLOW-DIAGRAM-VIEW-MODE | 10_flow_diagram, 09_screen | sourceとtests上は確認済み | manual verificationへ分離 | 実機Previewでselector、projection、viewport Fit、Applied Color Schemeを確認する | dogfoodモデル更新はCOV-015で完了 |
...
- COV-014Bでは、bulk Markdown copyを [[DATA-MW-DIAGNOSTIC-COPY-ACTION]] として整理した。all diagnostics / errors / warnings / notes はClipboardへ渡すMarkdown文字列のscopeであり、notesはinfo severityのUI分類である。
...
- v0.1.18 manual-edit guidanceは、診断detailsに何が起きているか、よくある原因、手動での直し方を表示する。invalid table header / row、unresolved reference、missing typeは手動確認とcopy支援の対象であり、Quick Fix MVPではない。
- v0.1.18 Markdown table handlingでは、全セルが空のdata rowだけを編集ノイズとして無視する。部分入力行は引き続きrequired-field diagnostics対象にする。
- COV-014Eでは、fully empty data rowはediting noiseとして除外し、partially filled rowはrequired-fieldやinvalid-table-row診断対象として残す線引きをdogfoodへ反映した。
- COV-014Aでは、lower panel tabsをViewer内Review UIとして整理した。Details / Relationships / Diagnostics / Source Links / Mermaid Sourceをtab化し、renderer switchingやBusiness Flow direction変更後も同一file / mode内で維持する。
...
- COV-013では、v0.1.19のFlow Diagram MVPを `dfd_diagram` とは別形式として整理した。`type: flow_diagram` は `schema: flow_diagram` として処理され、当時はDetail viewを中心に描画していた。
...
- COV-013Aでは、[[FLOW-MW-VIEWER-DIAGNOSTIC-QUICK-FIX]] を代表Dogfoodサンプルとして追加した。Flow Diagramは画面、UI action、処理境界、Markdownファイル間のcommunication / handoffを示すものであり、DFDやExplorer機能ではない。
...
- COV-015では、Mapping canonical headerが `target_ref / source_ref / transform / rule / required / notes` に変わったことを反映した。legacy source-firstは互換入力として受理されるが、新規dogfood実テーブルでは使わない。
- v0.1.20ではFlow DiagramのDetail / Screen viewが実装済みである。Screen viewはscreen / external / actor / user / context / messageをvisible nodeとして残し、内部node経由のedgeを射影する。
- `flow_view` は初期表示入力であり、toolbar切替後のeffective modeはfile scoped Viewer stateとして保持される。Markdown本文やfrontmatterは書き換えない。
- Flow Diagramでも `## Domain Sources` とlocal `## Domains` を使用でき、Domain name、parent、kindがgroup label、nested group、Color Scheme keyに使われる。
