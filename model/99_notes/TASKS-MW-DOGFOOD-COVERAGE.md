...
| COV-014D | v0.1.18 Quick Fix MVP | 要追加調査 | high | missing frontmatter id / name限定のQuick Fix MVPをdogfoodへどう反映するか | modeling-preview-view.ts | diagnostics screen, diagnostics rule, mapping, STATUS | Quick Fix actionはmissingFieldがtypeの場合は出ず、id / nameはbasenameから安全に値を作ってfrontmatterへ追加する | pending | existing frontmatter値を上書きしない範囲をsource-backedに確認して最小反映する | missing type、invalid table header、任意Markdown本文の自動修正ではない |
