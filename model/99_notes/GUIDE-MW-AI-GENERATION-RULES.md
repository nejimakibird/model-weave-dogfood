...
- **実装済みと断定しない**: `Source Links Explorer` や `Reference Explorer` は、現時点では Dogfood モデル上でのみ定義されている将来機能です。これらが本体に「実装済みである」ことを前提としたロジック提案は避けてください。
- **Source Links の位置付け**: 各モデル内の `## Source Links` セクションは、現時点ではMarkdown内の「設計メモ・追跡用リンク」です。これを自動解決して画面表示する `Source Links Explorer` は将来機能の設計モデルです。
