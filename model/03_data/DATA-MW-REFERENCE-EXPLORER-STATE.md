...
## Summary
Reference Explorerで表示する参照一覧、未解決参照、参照元/参照先、診断状態、フィルタ条件を保持するfuture / plannedの表示用状態データ。
[[DATA-MW-VAULT-MODEL-INDEX]] から導出される派生状態であり、Markdownモデルの正本ではない。
Model Weave 0.1.17時点では、Impact Summary / Relationship View / Weave Mapは実装済みだが、横断的なReference Explorer状態は本体srcで独立実装を確認できない。
...
- 本データはfuture / plannedのExplorer状態であり、現行Viewer内Relationship ViewやWeave Mapの派生状態とは分けて扱う。
...
| src/core/current-file-diagnostics.ts | symbol: buildCurrentObjectDiagnostics; kind: function; 未解決参照などの診断生成 |
| src/main.ts | symbol: openDiagnosticLocation; kind: function; 診断箇所ジャンプの参考 |
