...
| defaultFlowDiagramViewMode | Flow Diagram既定表示モード | string | | N | defaultFlowDiagramViewMode | [[DATA-MW-FLOW-DIAGRAM-VIEW-STATE]] | implemented。flow_diagram用。既定値 detail。許容値 detail / screen |
...
- defaultFlowDiagramViewMode は Flow Diagramのfrontmatter `flow_view` 未指定時だけ初期表示入力として使われる。toolbar切替後の実効modeやMarkdown本文を上書きしない。
...
| src/core/flow-diagram-view-mode.ts | symbol: resolveInitialFlowDiagramViewMode; kind: function; defaultFlowDiagramViewMode fallback |
