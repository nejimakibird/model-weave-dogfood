...
| src/renderers/app-process-business-flow.ts | buildAppProcessBusinessFlowMermaidSource | function | Business Flow Mermaid source生成とColor Scheme適用 |
| src/renderers/app-process-business-flow.ts | getStepShapeKind | function | Business Flow Step kindからshape種別を選ぶ |
| src/renderers/app-process-business-flow.ts | formatAppProcessStepNode | function | Business Flow Step node行を生成する |
| src/renderers/weave-map-mermaid.ts | buildWeaveMapMermaidSource | function | Weave Map Mermaid source生成とColor Scheme適用 |
| src/core/color-scheme.ts | resolveColorStyle | function | target / kindからColor Scheme styleを解決 |
| src/views/applied-color-scheme-renderer.ts | renderAppliedColorSchemeSectionContent | function | Applied Color Scheme sectionはViewer表示として扱う |
