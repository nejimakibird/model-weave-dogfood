...
## Summary
Source Links Explorerで表示するモデル資産と実装ソースの対応関係、検索条件、選択状態、関連診断を保持するfuture / plannedの表示用状態データ。
[[DATA-MW-VAULT-MODEL-INDEX]] から導出される派生状態であり、Markdownモデルの正本ではない。
Model Weave 0.1.17時点では、各モデル内の `## Source Links` 解析・Preview表示・Copy/Openは実装済みだが、横断的なSource Links Explorer状態は本体srcで独立実装を確認できない。
...
- 本データはfuture / plannedのExplorer状態であり、現行Preview内Source Links sectionの表示状態とは分けて扱う。
