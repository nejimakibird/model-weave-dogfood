...
| flowDiagramViewMode | Flow Diagram表示モード | string | | N | flowDiagramViewModes | [[DATA-MW-FLOW-DIAGRAM-VIEW-STATE]] | implemented / runtime。flow_diagramのDetail / Screen実効mode |
| flowDiagramViewModeState | Flow Diagram表示状態 | object | | N | flowDiagramViewModes | [[DATA-MW-FLOW-DIAGRAM-VIEW-STATE]] | implemented / runtime。filePathごとのmodeとinitializationKey |
...
| lowerPanelTabState | Lower Panel Tab状態 | object | | N | activeLowerPanelTabId | [[DATA-MW-LOWER-PANEL-TAB-STATE]] | implemented / runtime。Review情報のstable lower tabsとactive tab |
...
| defaultFlowDiagramViewMode | Flow Diagram既定表示モード | string | | N | viewerPreferences.defaultFlowDiagramViewMode | [[DATA-MW-FLOW-DIAGRAM-VIEW-STATE]] | implemented。frontmatter未指定Flow Diagramの初期mode |
...
- `flowDiagramViewMode` は Flow Diagramだけの実効表示モードであり、Detail / Screen切替に使う。toolbar選択はMarkdown本文を書き換えない。
- `flowDiagramViewModeState` はファイル単位に `{ mode, initializationKey }` を保持するViewer一時状態である。
...
- `lowerPanelTabState` は下部Review情報をDetails / Relationships / Diagnostics / Source Links / Mermaidのtabとして安定表示するViewer実行時状態である。
- lower panel tabsはgraph / non-graph review viewの両方で使われ、renderer switchingやBusiness Flow direction変更後も同一file / mode内ではtab構造を維持する。
...
- `defaultBusinessFlowDirection` / `defaultDomainsViewMode` / `defaultDomainDiagramViewMode` / `defaultFlowDiagramViewMode` は保存設定からViewerPreferencesへ渡される表示設定である。
- defaultFlowDiagramViewModeはfrontmatter `flow_view` 未指定時の初期化入力であり、toolbar切替後のeffective modeより優先されない。
...
| src/views/modeling-preview-view.ts | symbol: activeLowerPanelTabId; kind: field; lower panelの選択中tab |
| src/views/modeling-preview-view.ts | symbol: applyLowerPanelTabsToPane; kind: method; lower panel slotsをtab UIへ変換 |
...
| src/core/flow-diagram-view-mode.ts | symbol: FlowDiagramViewModeState; kind: class; Flow Diagram表示modeのファイル単位状態 |
| src/views/modeling-preview-view.ts | symbol: appendFlowDiagramViewSelector; kind: method; Viewer toolbarのFlow view selector |
