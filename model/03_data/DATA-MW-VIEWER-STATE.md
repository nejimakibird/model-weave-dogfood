...
| src/views/modeling-preview-view.ts | renderReviewSummaryPanel | method | Review Summary表示状態の構築 |
| src/views/modeling-preview-view.ts | createCollectionDiagramLowerPaneSlots | method | Review Panel下部slotsの構築 |
| src/views/modeling-preview-view.ts | renderObjectState | method | object modeのsplit shell下部構成 |
| src/views/modeling-preview-view.ts | renderSummaryState | method | summary modeのscreen preview / Business Flow構成 |
| src/views/modeling-preview-view.ts | renderDomainsState | method | domains modeの下部構成 |
| src/views/modeling-preview-view.ts | renderDomainDiagramState | method | domain_diagram modeの下部構成 |
| src/views/modeling-preview-view.ts | renderColorSchemeState | method | color_scheme modeの直接表示 |
| src/views/modeling-preview-view.ts | renderDfdObjectState | method | dfd-object modeのsplit shell下部構成 |
| src/views/modeling-preview-view.ts | renderDiagramState | method | diagram modeのlower slots構成 |
| src/views/modeling-preview-view.ts | renderRelationsState | method | relations modeの直接表示 |
| src/views/modeling-preview-view.ts | appendViewerFocusToolbar | method | focusMode UIの構築 |
| src/views/modeling-preview-view.ts | setViewOnlyMode | method | View Only一時状態の制御 |
...
| src/views/modeling-preview-view.ts | buildCurrentDiagramExportRenderable | method | pngExportAvailable相当の出力対象を準備する |
| src/views/modeling-preview-view.ts | openExportedPng | method | Export and OpenのOS側open処理 |
| src/main.ts | ModelWeavePlugin.getViewerPreferences | method | 保存設定からViewerPreferencesを構成 |
| src/settings/model-weave-settings.ts | ModelWeaveSettings | type | 現行実装済み設定項目 |
| src/settings/model-weave-settings.ts | DEFAULT_MODEL_WEAVE_SETTINGS | constant | defaultZoom / fontSize / nodeDensity の既定値 |
| src/settings/model-weave-settings.ts | ModelWeaveViewerPreferences | type | Viewerへ渡される表示設定 |
...
| src/core/color-scheme.ts | AppliedColorSchemeRow | interface | Applied Color Scheme表示行 |
...
| src/renderers/weave-map-mermaid.ts | resolveLayerStyle | function | target=weave_mapのlayer color overrideとfallback |
| src/core/impact-analyzer.ts | buildImpactSummary | function | Impact Summary生成 |
| src/renderers/app-process-business-flow.ts | buildAppProcessBusinessFlowInteractionTargets | function | ref-aware Business Flow step hover / click targetの構築 |
| src/views/modeling-preview-view.ts | handleAppProcessFlowConnectStepClick | method | Flow接続モードのStepクリック処理 |
| src/core/app-process-flow-editor.ts | addAppProcessFlow | function | app_process `## Flows` 行の追加と結果判定 |
...
| src/views/mermaid-node-interactions.ts | attachMermaidNodeInteractions | function | Business Flow Stepのhover preview / click targetをSVGへ付与 |
| src/renderers/source-links-renderer.ts | renderSourceLinks | function | Source Links sectionのPreview表示とCopy Path / Open操作 |
| src/core/impact-analyzer.ts | collectRelatedSourceLinks | function | Relationship / impact summaryに関連Source Linksを集約 |
| src/core/weave-map.ts | buildWeaveMapModel | function | ImpactSummaryからWeave Map派生モデルを生成 |
| src/renderers/weave-map-mermaid.ts | buildWeaveMapMermaidSource | function | Weave Map Mermaid source生成とlayer coloring |
