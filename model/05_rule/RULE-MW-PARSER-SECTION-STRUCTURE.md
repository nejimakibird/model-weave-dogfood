...
| Mappings | Required | `target_ref / source_ref / transform / rule / required / notes` | v0.1.20 canonical。source-firstはlegacy互換として受理される |
...
- Mappingの現行canonical headerは `target_ref / source_ref / transform / rule / required / notes` である。legacy `source_ref / target_ref / transform / rule / required / notes` は互換入力として受理されるが、AI生成時の推奨形式にしない。
- `source_ref` や `target_ref` など、意味が近い列であっても、FORMATまたはparser実装が定義した列の順序を勝手に入れ替えてはいけません。
...
| src/parsers/mapping-parser.ts | symbol: MAPPING_HEADERS; kind: constant; Mapping canonical target-first header |
| src/parsers/mapping-parser.ts | symbol: LEGACY_MAPPING_HEADERS; kind: constant; Mapping legacy source-first compatibility |
