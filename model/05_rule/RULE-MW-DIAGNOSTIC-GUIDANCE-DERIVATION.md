...
| G21 | invalid table row diagnostic | warning | 行内容を手動確認する | 部分入力行、余分なpipe、列数不一致を確認する |
| G22 | unresolved reference diagnostic | warning | 参照先を手動確認する | target ID、Wikilink、参照先file、indexed model setを確認する |
| G23 | fully empty table data row | info | 診断対象にしない | 全セル空白のデータ行はediting noiseとして無視する |
...
- invalid table rowでは部分入力行、余分なpipe、列数不一致、required field不足を手動で確認する。
- fully empty data rowは、全セルが空または空白のみのデータ行であり、v0.1.18以降はediting noiseとして診断対象外になり得る。
- partially filled rowは無視されず、required-fieldやinvalid-table-rowの診断対象として残る。
