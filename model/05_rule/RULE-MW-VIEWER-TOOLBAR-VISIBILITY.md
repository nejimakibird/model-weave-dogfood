...
|---|---|---|---|
| src/views/modeling-preview-view.ts | appendRendererSelection | method | Renderer selector の表示制御 |
| src/views/modeling-preview-view.ts | renderCurrentState | method | Viewer状態に応じたUI描画 |
| src/core/render-mode.ts | getSupportedRenderModes | function | 対象モデルで利用可能な描画モード取得 |
| src/core/render-mode.ts | resolveRenderMode | function | 実効描画モードの解決 |