...
## Summary
Model Weaveモデル資産と実装ソースコードの対応関係を横断一覧表示するfuture / plannedの画面。
Source Linksに記述されたpath / symbol / kind / notesを確認し、対象モデル資産や実装ソースへの追跡を支援する。
Model Weave 0.1.17時点では、各モデル内の `## Source Links` 解析・Preview表示・Copy/Openは実装済みだが、独立したSource Links Explorer Viewは本体srcで確認できない。
...
## Notes
- このScreenはfuture / plannedのSource Links横断確認UIであり、Markdownモデルの正本を変更しない。
- [[DATA-MW-SOURCE-LINK-EXPLORER-STATE]] は [[DATA-MW-VAULT-MODEL-INDEX]] から導出される派生状態である。
- 現行実装済みのSource Links section supportはPreview内の `## Source Links` 表示とCopy/Openであり、このExplorer画面とは別機能である。
- Relationship View / Impact SummaryのRelated Source Links集約は実装済みだが、Explorer専用の一覧・検索・選択状態ではない。
