...
## Summary
Model Weave Viewer全体のレイアウトを定義し、上部ツールバー、中央プレビュー領域、下部Review Panel tabsの関係を明確にする。
...
| toolbar | Viewer Toolbar | toolbar | ズーム、レンダラー選択、Flow Diagram表示切替などの操作を含む上部ツールバー | focus buttonと各図のzoom toolbarを含む |
...
| review_panel_area | Review Panel Area | content-pane | Review Summary、Diagnostics、Relationships / Impact、Source Links、Weave Map、Details、Mermaid Sourceを表示する下部領域 | implemented。stable lower panel tabsを含む |
| lower_panel_tabs | Lower Panel Tabs | tabs | Review情報を安定したtabとして表示する | graph / non-graph review viewの両方で使用 |
...
| flowDiagramViewSelector | Flow view Selector | select | toolbar | string | N | [[DATA-MW-FLOW-DIAGRAM-VIEW-STATE]].mode | [[RULE-MW-FLOW-DIAGRAM-VIEW-STATE]] | implemented。flow_diagram表示時だけDetail / Screenを切り替える |
...
| reviewPanelArea.slotStructure | Review Panel Slot Structure | object | review_panel_area | object | N | [[DATA-MW-VIEWER-STATE]].reviewPanelSections | | implemented。modeごとの下部slot構成 |
| reviewPanelArea.modeLayout | Review Panel Mode Layout | object | review_panel_area | object | N | [[DATA-MW-VIEWER-STATE]].modeLowerPaneLayout | | implemented。modeごとのbottomPaneまたはlower slots |
| reviewPanelArea.availableTabs | Review Panel Available Tabs | object | lower_panel_tabs | object | N | [[DATA-MW-LOWER-PANEL-TAB-STATE]].availableTabs | | implemented。表示可能なlower panel tabs |
| reviewPanelArea.activeTab | Review Panel Active Tab | object | lower_panel_tabs | string | N | [[DATA-MW-LOWER-PANEL-TAB-STATE]].activeTab | | implemented。選択中のlower panel tab |
| lowerPanelTabs | Lower Panel Tabs | tabs | lower_panel_tabs | object | N | [[DATA-MW-LOWER-PANEL-TAB-STATE]] | | implemented。Details / Relationships / Diagnostics / Source Links / Mermaid Sourceをtab化 |
...
| copyImpactSummaryButton | Copy Impact Summary Button | button | review_panel_area | action | N | [[DATA-MW-IMPACT-SUMMARY]].formattedMarkdown | | implemented。関連サマリをMarkdown文字列としてコピー |
| weaveMapBlock | Weave Map Block | panel | review_panel_area | object | N | [[DATA-MW-VIEWER-STATE]].weaveMapMermaidSource | | implemented。Relationship View内の派生視覚化 |
| relatedSourceLinksList | Related Source Links List | list | review_panel_area | object | N | [[DATA-MW-IMPACT-SUMMARY]].relatedSourceLinks | | implemented。self / inbound / outbound由来の関連Source Linksを表示 |
...
| changeFlowDiagramView | Change Flow Diagram View | field_event | flowDiagramViewSelector | change | [[PROC-MW-FLOW-DIAGRAM-VIEW-SWITCH]] | | [[RULE-MW-FLOW-DIAGRAM-VIEW-STATE]] | Detail / Screenを切り替え、Markdown本文は変更しない |
...
| switchLowerPanelTab | Switch Lower Panel Tab | ui_action | lowerPanelTabs | click | applyLowerPanelTabsToPane | | | Viewer内のactive lower tabを切り替える |
...
| flow-diagram | Flow Diagram graph | diagram lower slots | Review Summary、Diagnostics、Source Links、Details、Mermaid Source、Applied Color Scheme | Flow view selectorはflow_diagram時だけ表示 | Detail / Screenの実効modeをrendererへ渡す |
...
- `flowDiagramViewSelector` は `schema: flow_diagram` のときだけ表示され、Detail / Screenの実効modeをViewer stateへ保存する。
- Flow viewのtoolbar切替はMarkdown本文やfrontmatter `flow_view` を変更しない。
- Flow view切替時はViewport cacheを破棄し、再描画後にFitする。
...
- lower panel tabsはReview Panel内のDetails / Relationships / Diagnostics / Source Links / Mermaid Sourceを安定したtabとして表示する。
- lower panel tabsはgraph model review viewsだけでなく、objectやsummaryなどのnon-graph review viewにも適用される。
- renderer switchingやBusiness Flow direction変更で再描画されても、同一file / mode内ではlower tab構造とactive tabを維持する。
- file pathまたはPreviewState.modeが変わる場合は、active tabをリセットし得る。
- lower panel tabsはSource Links Explorer / Reference Explorer / Model Index Viewではなく、Viewer内のReview UIである。
- lower panel tabsはMarkdown本文を変更せず、PNG export対象にも含めない。
...
- Flow DiagramのScreen viewでは、screen / external / actor / user / context / message のvisible nodeだけを残し、内部node経由の到達関係をprojected edgeとして表示する。
...
| src/views/modeling-preview-view.ts | symbol: applyLowerPanelTabsToPane; kind: method; lower panel slotsをtab UIへ変換する |
| src/views/modeling-preview-view.ts | symbol: resolveActiveLowerPanelTab; kind: method; active lower tabの維持またはfallback |
