...
|---|---|---|---|
| source/model-weave-repo/src/views/modeling-preview-view.ts | ModelingPreviewView | class | Viewerのメインビュークラス |
| source/model-weave-repo/src/views/modeling-preview-view.ts | renderCurrentState | method | 現在のプレビュー状態をレンダリングする |
| source/model-weave-repo/src/views/modeling-preview-view.ts | renderObjectState | method | オブジェクトモデルのプレビューをレンダリングする |
| source/model-weave-repo/src/views/modeling-preview-view.ts | renderSummaryState | method | サマリーモデルのプレビューをレンダリングする |
| source/model-weave-repo/src/views/modeling-preview-view.ts | renderDfdObjectState | method | DFDオブジェクトモデルのプレビューをレンダリングする |
| source/model-weave-repo/src/views/modeling-preview-view.ts | renderDiagramState | method | ダイアグラムモデルのプレビューをレンダリングする |
| source/model-weave-repo/src/views/modeling-preview-view.ts | createViewerSplitShell | method | 上下分割パネルのUIシェルを作成する |
| source/model-weave-repo/src/views/modeling-preview-view.ts | appendRendererSelection | method | レンダラー選択UIをツールバーに追加する |
| source/model-weave-repo/src/views/modeling-preview-view.ts | renderDiagnostics | function | 診断情報をレンダリングするユーティリティ関数 |
| source/model-weave-repo/src/core/render-mode.ts | resolveRenderMode | function | レンダラーモードを解決する |
| source/model-weave-repo/src/core/render-mode.ts | getSupportedRenderModes | function | サポートされているレンダラーモードを取得する |
| source/model-weave-repo/src/main.ts | syncPreviewToActiveFile | method | アクティブファイルとプレビューを同期する |
| source/model-weave-repo/src/main.ts | openDiagnosticLocation | method | 診断箇所にジャンプする |