...
| diagnosticsPanel | Diagnostics Panel | panel | review_panel_area |  | N | [[SCR-MW-VIEWER-DIAGNOSTICS-PANEL]] | [[RULE-MW-RENDERER-DIAGNOSTICS-SEVERITY-MAP]] | 診断メッセージを表示する下部パネル |
...
| color-scheme | color scheme details | lower paneなし | Diagnostics、Colors table | Review Summary、Relationship View、Source Linksは表示しない | color_scheme自身の定義確認画面 |
...
| src/views/modeling-preview-view.ts | renderReviewSummaryPanel | method | Review Summary chip群を表示する |
| src/views/modeling-preview-view.ts | createCollectionDiagramLowerPaneSlots | method | collection diagramの下部Review Panel slotsを作成する |
| src/views/modeling-preview-view.ts | moveDetailSections | method | Mermaid renderer由来のdetails sectionを下部details slotへ移動する |
| src/views/modeling-preview-view.ts | appendViewerFocusToolbar | method | Viewer全体のfocus modeボタンを作成する |
| src/views/modeling-preview-view.ts | appendViewOnlyControl | method | 各図のView Onlyボタンをzoom toolbarへ追加する |
| src/views/modeling-preview-view.ts | setViewOnlyMode | method | View Only stageへの移動と復帰を制御する |
...
| src/views/modeling-preview-view.ts | renderDomainsState | method | domains modeの下部表示を構成する |
| src/views/modeling-preview-view.ts | renderDomainDiagramState | method | domain_diagram modeの下部表示を構成する |
| src/views/modeling-preview-view.ts | renderColorSchemeState | method | color_scheme modeを直接表示する |
...
| src/views/modeling-preview-view.ts | renderRelationsState | method | relations modeを直接表示する |
...
| src/views/modeling-preview-view.ts | updateAppProcessFlowConnectButton | method | Flow接続ボタンの表示状態を更新する |
| src/views/modeling-preview-view.ts | createAppProcessFlowConnectStatus | method | 接続元Stepの選択状態を表示する |
| src/views/modeling-preview-view.ts | handleAppProcessFlowConnectStepClick | method | Business Flow StepクリックからFlow接続処理を実行する |
| src/core/app-process-flow-editor.ts | addAppProcessFlow | function | app_process `## Flows` 行を追加する |
| src/core/app-process-step-interaction-target.ts | resolveAppProcessStepInteractionTarget | function | Stepのref-aware hover / click targetを解決する |
| src/views/mermaid-node-interactions.ts | attachMermaidNodeInteractions | function | Mermaid SVGノードへhover / click動作を付与する |
...
| src/views/modeling-preview-view.ts | renderImpactSummarySection | method | Relationship View sectionを表示する |
| src/views/modeling-preview-view.ts | renderWeaveMapBlock | method | Relationship View内のWeave Mapを表示する |
| src/renderers/mermaid-shared.ts | renderMermaidSourceIntoShell | function | Mermaid SourceとRender Debug panelを生成する |
| src/core/impact-analyzer.ts | formatImpactSummaryAsMarkdown | function | Copy Impact Summary用Markdownを生成する |
| src/core/weave-map.ts | buildWeaveMapModel | function | ImpactSummaryからWeave Map派生モデルを生成する |
| src/renderers/weave-map-mermaid.ts | buildWeaveMapMermaidSource | function | Weave Map Mermaid sourceを生成する |
| src/views/modeling-preview-view.ts | renderAppliedColorScheme | method | Applied Color Scheme sectionを下部ペインに表示する |
| src/views/applied-color-scheme-renderer.ts | renderAppliedColorSchemeSectionContent | function | Applied Color Schemeのsummaryとtableを描画する |
| src/core/color-scheme.ts | getAppliedColorSchemeRowsForTargets | function | view targetに対応するApplied Color Scheme表示行を作る |
| src/core/color-scheme.ts | resolveDefaultColorScheme | function | defaultColorSchemeRefの未設定 / 未解決 / 不正type fallbackを扱う |
| src/export/png-export.ts | exportDiagramRenderableAsPng | function | 描画済み図をPNGとして出力する |
| src/export/png-export.ts | buildDomDiagramExportSnapshot | function | export surfaceとboundsを取得する |
| src/views/modeling-preview-view.ts | openExportedPng | method | PNG出力後にOS側でファイルを開く |
| src/views/modeling-preview-view.ts | buildCurrentDiagramExportRenderable | method | 現在のPreview状態からPNG出力用renderableを作る |
| src/views/modeling-preview-view.ts | exportCurrentDiagramAsPngWithNotice | method | PNG出力操作と通知を実行する |
| src/views/modeling-preview-view.ts | exportCurrentDiagramAsPngAndOpenWithNotice | method | PNG出力後にopenExportedPngへ進む |
| src/renderers/zoom-toolbar.ts | createZoomToolbar | function | PNG出力ボタンとExport and Openボタンを作る |
