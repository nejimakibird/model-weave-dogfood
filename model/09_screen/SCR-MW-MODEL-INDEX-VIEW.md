...
Vault内で検出されたModel Weaveモデル資産を一覧表示するfuture / plannedの画面。
モデル種別、ファイルパス、診断状態、未解決参照数などを確認し、対象モデルファイルへ移動できるようにする。
Model Weave 0.1.17時点では、Vault Index rebuildやViewer内部の索引利用は実装済みだが、独立したModel Index Viewは本体srcで確認できない。
...
|---|---|---|---|---|---|---|---|---|
| modelFileCountLabel | モデルファイル数 | label | index_summary_area | number | N | [[DATA-MW-VAULT-MODEL-INDEX]].modelsByFilePath | | 検出されたMarkdownモデルファイル数 |
| modelAssetCountLabel | モデル資産数 | label | index_summary_area | number | N | [[DATA-MW-VAULT-MODEL-INDEX]].modelsByFilePath | | 認識されたModelAsset相当の件数 |
| unresolvedReferenceCountLabel | 未解決参照数 | label | index_summary_area | number | N | [[DATA-MW-VAULT-MODEL-INDEX]].warningsByFilePath | [[RULE-MW-RENDERER-DIAGNOSTICS-SEVERITY-MAP]] | unresolved reference数 |
| diagnosticSummaryLabel | 診断サマリー | label | index_summary_area | string | N | [[DATA-MW-VAULT-MODEL-INDEX]].warningsByFilePath | [[RULE-MW-RENDERER-DIAGNOSTICS-SEVERITY-MAP]] | error / warning件数 |
| modelTypeFilter | モデル種別フィルタ | select | index_filter_area | string | N | [[DATA-MW-VAULT-MODEL-INDEX]].modelsByFilePath | | data_object / rule / mapping 等 |
...
| modelAssetList | モデル資産一覧 | list | model_list_area | object | Y | [[DATA-MW-VAULT-MODEL-INDEX]].modelsByFilePath | | 複数のParsedFileModelを一覧表示 |
...
- このScreenはfuture / plannedのVaultモデルインデックス表示用UIであり、Markdownモデルの正本を変更しない。
- 表示対象の正本はVault内Markdownファイルである。
- [[DATA-MW-VAULT-MODEL-INDEX]] は派生インデックスであり、ファイル更新や再解析により再生成される。
- 現行実装済みのVault IndexはPreview、Relationship View、診断などの内部処理に使われる派生索引であり、この独立一覧画面とは分けて扱う。
...
| src/views/modeling-preview-view.ts | symbol: renderCurrentState; kind: method; Viewer表示更新 |
