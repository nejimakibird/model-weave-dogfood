...
現行実装では Notes / Warnings / Errors に分類し、診断カードごとにメタ情報、詳細、開く操作、コピー支援を表示する。
...
| quickFixButton | Quick Fix Button | button | diagnostic_action_area | action | N | | | future。現行実装ではMarkdown自動修正はしない |
...
- コピー支援は実装済みだが、Markdown本文を直接変更するQuick Fixや自動修正ではない。
- Quick Fixや自動修正ボタンは、現行srcで確認できないためfutureとして扱う。
...
| src/views/modeling-preview-view.ts | getDiagnosticMetadata | function | 診断メタ情報の抽出 |
| src/views/modeling-preview-view.ts | getDiagnosticDetailEntries | function | 診断詳細情報の抽出 |
| src/views/modeling-preview-view.ts | getDiagnosticReferenceValue | function | 参照値コピーの対象を抽出 |
| src/views/modeling-preview-view.ts | getExpectedHeaderForDiagnostic | function | 期待ヘッダーコピーの対象を抽出 |
| src/views/modeling-preview-view.ts | getFrontmatterExampleForDiagnostic | function | frontmatter例を生成 |
| src/views/modeling-preview-view.ts | formatDiagnosticAsMarkdown | function | 診断Markdownコピー用文字列 |
| src/main.ts | openDiagnosticLocation | function | 診断対象へのジャンプ |
| src/core/domain-diagnostics.ts | formatAppProcessUnknownDomainMessage | function | Business Flow Domain診断メッセージ |
| src/core/domain-diagram-resolver.ts | resolveDomainSources | function | Domain Sources診断の生成 |
| src/core/app-process-domain-resolver.ts | resolveAppProcessDomainPlacement | function | Steps.domain診断の生成 |
