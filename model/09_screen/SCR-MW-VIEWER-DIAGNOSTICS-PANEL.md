...
Viewerの下部Review PanelのDiagnostics tabに表示され、現在のファイルのバリデーション結果やエラー情報を一覧表示するパネル。
現行実装では Notes / Warnings / Errors に分類し、Diagnostics tab上部にbulk Markdown copy actionsを表示し、診断カードごとにメタ情報、詳細、手動修復ガイダンス、開く操作、コピー支援、missing frontmatter id / name 限定のQuick Fix MVPを表示する。
...
| diagnostic_bulk_action_area | 一括コピー領域 | container | Diagnostics tab全体のbulk copy actions | Quick Fixとは別グループ |
...
| diagnosticManualGuidance | 手動修復ガイダンス | panel | diagnostic_list_area | object | N | [[DATA-MW-DIAGNOSTIC-MANUAL-GUIDANCE]] | [[RULE-MW-DIAGNOSTIC-GUIDANCE-DERIVATION]] | what happened / cause / manual fixを表示 |
| copyAllDiagnosticsButton | すべてコピー | button | diagnostic_bulk_action_area | action | N | [[DATA-MW-DIAGNOSTIC-COPY-ACTION]] | [[RULE-MW-DIAGNOSTIC-GUIDANCE-DERIVATION]] | implemented。all diagnosticsをMarkdownとしてコピー |
| copyErrorsButton | エラーをコピー | button | diagnostic_bulk_action_area | action | N | [[DATA-MW-DIAGNOSTIC-COPY-ACTION]] | [[RULE-MW-DIAGNOSTIC-GUIDANCE-DERIVATION]] | implemented。errorsがある場合だけ表示 |
| copyWarningsButton | 警告をコピー | button | diagnostic_bulk_action_area | action | N | [[DATA-MW-DIAGNOSTIC-COPY-ACTION]] | [[RULE-MW-DIAGNOSTIC-GUIDANCE-DERIVATION]] | implemented。warningsがある場合だけ表示 |
| copyNotesButton | ノートをコピー | button | diagnostic_bulk_action_area | action | N | [[DATA-MW-DIAGNOSTIC-COPY-ACTION]] | [[RULE-MW-DIAGNOSTIC-GUIDANCE-DERIVATION]] | implemented。info severityのnotesがある場合だけ表示 |
...
| copyAllDiagnostics | すべての診断をコピー | ui_action | copyAllDiagnosticsButton | click | navigator.clipboard.writeText | | [[MAP-MW-RENDERER-DIAGNOSTIC-TO-UI]] | all diagnosticsをMarkdownでコピー |
| copyErrors | エラーをコピー | ui_action | copyErrorsButton | click | navigator.clipboard.writeText | | [[MAP-MW-RENDERER-DIAGNOSTIC-TO-UI]] | errorsだけをMarkdownでコピー |
| copyWarnings | 警告をコピー | ui_action | copyWarningsButton | click | navigator.clipboard.writeText | | [[MAP-MW-RENDERER-DIAGNOSTIC-TO-UI]] | warningsだけをMarkdownでコピー |
| copyNotes | ノートをコピー | ui_action | copyNotesButton | click | navigator.clipboard.writeText | | [[MAP-MW-RENDERER-DIAGNOSTIC-TO-UI]] | info severityのnotesだけをMarkdownでコピー |
...
- Diagnostics Panelはlower panel tabs内のDiagnostics tabとして表示される。独立Explorer画面ではない。
- renderer switchingやBusiness Flow direction変更でPreviewが再描画されても、Viewer下部のtab構造は安定維持される。
...
- manual-edit guidanceはdetails内の表示であり、何が起きているか、よくある原因、手動での直し方を示す。
- invalid table headerでは期待ヘッダーの確認、invalid table rowでは空行や列数の確認、unresolved referenceでは参照先IDやWikilinkの確認を促す。
- manual-edit guidanceはCopy Expected Header / Copy Frontmatter Example / Copy Referenceと併用できるが、Markdown本文を自動更新しない。
- bulk Markdown copy actionsはDiagnostics tab上部のaction groupであり、all diagnostics / errors / warnings / notesをMarkdown文字列としてClipboardへ渡す。
- notesは `ValidationWarning.severity` が `info` の診断をViewer上で表示する分類名であり、保存値としての `note` severityは使わない。
- bulk copyは個別診断カードのCopy Message / Copy Markdown / Copy Reference / Copy Expected Header / Copy Frontmatter Exampleとは別の操作である。
...
- bulk copyはQuick Fix MVPとは別操作であり、Markdown本文を変更しない。
...
| src/views/modeling-preview-view.ts | symbol: renderDiagnosticsBulkActions; kind: function; Diagnostics tabのbulk copy actionsを生成 |
| src/views/modeling-preview-view.ts | symbol: formatDiagnosticsBulkMarkdown; kind: function; 診断一覧をMarkdown文字列に整形 |
...
| src/views/modeling-preview-view.ts | symbol: getDiagnosticGuidanceEntries; kind: function; 手動修復ガイダンスの抽出 |
