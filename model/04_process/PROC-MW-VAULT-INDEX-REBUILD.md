...
|---|---|---|---|
| source/model-weave-repo/src/core/vault-index.ts | VaultIndex | class | Vault内モデル資産の索引管理 |
| source/model-weave-repo/src/core/schema-detector.ts | detectModelType | function | Markdownファイル種別の自動検出 |
| source/model-weave-repo/src/core/relation-resolver.ts | resolveDiagramRelations | function | モデル間参照の解決ロジック |
| source/model-weave-repo/src/core/current-file-diagnostics.ts | buildCurrentObjectDiagnostics | function | 診断情報の解析と生成 |