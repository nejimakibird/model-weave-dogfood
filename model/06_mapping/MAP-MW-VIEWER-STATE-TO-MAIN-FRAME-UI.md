...
| reviewPanelSections | [[SCR-MW-VIEWER-MAIN-FRAME]].reviewPanelArea.slotStructure | modeに応じて下部Review Panelのslot構成を決める | | N | implemented。collection diagramでは専用slotsを使う |
| lowerPanelTabState.availableTabs | [[SCR-MW-VIEWER-MAIN-FRAME]].reviewPanelArea.availableTabs | available tabsをViewer下部tab UIへ反映 | | N | implemented。lower panel tabsの構成要素 |
| lowerPanelTabState.activeTab | [[SCR-MW-VIEWER-MAIN-FRAME]].reviewPanelArea.activeTab | active tabをViewer下部tab UIへ反映 | | N | implemented。renderer switchingやBusiness Flow direction変更後も安定維持 |
| modeLowerPaneLayout | [[SCR-MW-VIEWER-MAIN-FRAME]].reviewPanelArea.modeLayout | PreviewState.modeごとのrender methodでbottomPane、lowerSlots、またはlower paneなしを選ぶ | | N | implemented。mode別Review Panel差分 |
